...
def get_time_function(**context):
    url = f"{config['search_engine_endpoint_url']}/last-modified-property"

    try:
        resp = requests.get(url, timeout=10)
        resp.raise_for_status()
        modified_time = resp.json().get("modified_time")
    ...
    if modified_time:
        ...
        iran_dt = dt.astimezone(TEHRAN_TZ).replace(tzinfo=None) - timedelta(hours=1)
        logging.info(f"Last modified (DB Iran time): {iran_dt}")

        return iran_dt

    fallback_dt = (datetime.now(TEHRAN_TZ) - timedelta(days=1)).replace(tzinfo=None)
    logging.warning(f"No modified_date found. Using fallback: {fallback_dt}")
    return fallback_dt
...
def load_function(ti, **context):
    ...
    send_batches(endpoint=ENDPOINT_URL, items=rows, batch_size=BATCH_SIZE)
...
with DAG(
    ...
    catchup=False,
    max_active_runs=1,
    tags=["extract data","delta database","incremental", "search-engine"],
) as dag:
