...
def fa_to_en(text):
    ...
    for c in text:
        chr_code = ord(c)
        new_text += (
            chr(chr_code - 1728) if (chr_code >= 1776 and chr_code <= 1785) else c
        )
    return new_text
...
df = pd.read_excel(
    f"C:/Users/m.khodadadi/Documents/GitHub/divar_data_mapping/data/{city}.xlsx"
)
...
elevator_mapping = {0: 1091, 1: 1090}
...
loan_mapping = {0: 1097, 1: 1096}
df["Loan"] = df["Loan"].map(loan_mapping)

parking_mapping = {0: 1095, 1: 1094}
df["Parking"] = df["Parking"].map(parking_mapping)

rooms_mapping = {"بدون اتاق": 1152, 1: 1153, 2: 1154, 3: 1155, 4: 1156, 5: 1157}
df["Rooms"] = df["Rooms"].map(rooms_mapping)

warehouse_mapping = {0: 1093, 1: 1092}
df["WareHouse"] = df["WareHouse"].map(warehouse_mapping)

category_mapping = {"فروش مسکونی": 6, "اجارهٔ مسکونی": 7}
...
df_custom_field = pd.concat(
    [df_custom_field, pd.DataFrame(new_rows)], ignore_index=True
)
